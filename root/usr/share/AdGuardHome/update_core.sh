...
check_wgetcurl() {
    ...
    [ -z "$1" ] && (apk del wget; apk add wget; check_wgetcurl 1; return)
...
doupx() {
    Archt="$(apk info linux-headers | grep Architecture | awk -F "[ _]" '{print($2)}')"
    case $Archt in
        "x86")
            Arch="i386"
            ;;
        ...
        "mipsel")
            ...
            ;;
        *)
            echo -e "error not support $Archt if you can use official release please issue a bug"
            EXIT 1
            ;;
    esac
